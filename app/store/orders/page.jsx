...
export default function StoreOrders() {
    ...
    const fetchOrders = async () => {
        try {
            const { data } = await axios.get('/api/store/orders/getOrders')
    ...
    const updateOrderStatus = async (orderId, status) => {
        await axios.post('/api/store/orders/updateStatus', { orderId, status })
        await fetchOrders()
    }
                ...
                <div onClick={closeModal} className="fixed inset-0 flex items-center justify-center bg-black/50 text-slate-700 text-sm backdrop-blur-xs z-50" >
                    <div onClick={e => e.stopPropagation()} className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative">
                        ...
                        <div className="mb-4">
                            ...
                            <p><span className="text-green-700">Address:</span> {`${selectedOrder.address?.street}, ${selectedOrder.address?.city}, ${selectedOrder.address?.state}, ${selectedOrder.address?.zipCode}, ${selectedOrder.address?.country}`}</p>
                        </div>
